...
class FacultiesDataRepository(
    ...
    private val cache = BehaviorSubject.create<List<Faculty>>()

    override fun getAll(): Single<List<Faculty>> = if (cache.hasValue()) cache.singleOrError() else fetch()
    ...
    private fun fetch() = facultiesDataSource.getFaculties().doOnSuccess(cache::onNext)
