...
class BaseSelectionStepAdapter<Item>(
    ...
    companion object {
        fun <T> createDiffCallback(converter: ItemTitleProvider<T>) = object : SimpleDiffCallback2<T>() {
            override fun areContentsTheSame(oldItem: T, newItem: T): Boolean = converter.invoke(oldItem) == converter.invoke(newItem)
        }
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.item_simple_text, parent, false) as TextView
